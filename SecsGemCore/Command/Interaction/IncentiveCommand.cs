...
namespace SecsGem.Core.Command.Interaction
{
    /// <summary>
    ...
    {
        private readonly string IncentiveCommandExcelPath = Path.Combine(PFCommonParame.PFFileParame.ConfigPath, "SecsGemCommandConfig.xlsx");
        ...
        /// 添加单个命令
        /// </summary>
        public Task<bool> AddCommand(SFCommand command)
        {
            if (command == null || !ValidateCommand(command).GetAwaiter().GetResult())
            {
            ...
            {
                return Task.FromResult(false); // 已存在
            }
        ...
        public Task<bool> ValidateCommand(SFCommand command)
        {
            ...
            // 验证奇数类主动指令（根据需求）
            bool isOddFunction = (command.Function % 2) == 1;

            return Task.FromResult(isValid && isOddFunction);
        }
